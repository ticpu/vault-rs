...
it covers the calling thread and its children and not siblings. A kernel without it warns and
continues — the tests are correct about their paths regardless, and what is lost is the backstop.
`these_tests_cannot_write_outside_their_scratch_directory` is what keeps the claim honest: it
probes just outside the boundary before the real home, so a confinement that stopped working is
caught by the harmless probe.
