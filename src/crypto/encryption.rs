...
impl EncryptionManager {
    pub fn new(vault_addr: String) -> Self {
        Self {
            key_manager: KeyManager::new(vault_addr),
        }
    }
    ...
    pub async fn encrypt_data(
        ...
        let master_key = self.key_manager.get_master_key(vault_token).await?;
    ...
    pub async fn encrypt_to_file<P: AsRef<Path>>(
        ...
        let encrypted_data = self.encrypt_data(vault_token, data, context).await?;
    ...
    pub async fn decrypt_from_file<P: AsRef<Path>>(
        ...
        self.decrypt_data(vault_token, &encrypted_data, context)
            .await
    }

    /// Initialize encryption key in personal vault
    pub async fn init_encryption_key(&self, vault_token: &str) -> Result<()> {
        self.key_manager.init_encryption_key(vault_token).await
    }
    ...
    pub async fn encrypt_string(
        ...
        self.encrypt_data(vault_token, data.as_bytes(), context)
            .await
    }
    ...
    pub async fn decrypt_string(
        ...
        let decrypted_bytes = self
            .decrypt_data(vault_token, encrypted_data, context)
            .await?;
    ...
    pub async fn encrypt_yaml<T: serde::Serialize>(
        ...
        self.encrypt_string(vault_token, &yaml_string, context)
            .await
    }
    ...
    pub async fn decrypt_yaml<T: serde::de::DeserializeOwned>(
        ...
        let yaml_string = self
            .decrypt_string(vault_token, encrypted_data, context)
            .await?;
    ...
    pub async fn encrypt_yaml_to_file<T: serde::Serialize, P: AsRef<Path>>(
        ...
        let encrypted_data = self.encrypt_yaml(vault_token, data, context).await?;
    ...
    pub async fn decrypt_yaml_from_file<T: serde::de::DeserializeOwned, P: AsRef<Path>>(
        ...
        let encrypted_data = fs::read(&file_path)?;
        self.decrypt_yaml(vault_token, &encrypted_data, context)
            .await
    }
}
